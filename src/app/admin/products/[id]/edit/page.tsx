...
export default async function EditProductPage({ params }: Props) {
  ...
  const gallery = (product.gallery_urls ?? []).join("\n");

  return (
    <div className="space-y-6">
      <h1 className="font-display text-3xl text-background">Edit product</h1>
      <form
      ...
      >
        ...
        <Field
          ...
          name="price_krw"
          type="number"
          defaultValue={String(product.price_krw)}
          required
        />
        <Field
          label="Compare-at (KRW)"
          name="compare_at_krw"
          type="number"
          defaultValue={
            product.compare_at_krw != null ? String(product.compare_at_krw) : ""
          }
        />
        <Field label="SKU" name="sku" defaultValue={product.sku ?? ""} />
        <Field label="Stock" name="stock" type="number" defaultValue={String(product.stock)} required />
        <Field
        ...
        />
        <Field label="Primary image URL" name="image_url" defaultValue={product.image_url} required />
        <div>
          <label className="text-xs uppercase tracking-widest text-background/60">
